...
export const Route = createFileRoute("/platform/settings")({
  beforeLoad: () => {
    if (window.localStorage.getItem("biznexus-demo-role") !== "PlatformAdmin") throw redirect({ to: "/platform/admin/login" });
  },
  component: PlatformSettingsPage,
});

function PlatformSettingsPage() {
  const [name, setName] = useState("BizNexus Platform");
  const [supportEmail, setSupportEmail] = useState("support@biznexus.local");
