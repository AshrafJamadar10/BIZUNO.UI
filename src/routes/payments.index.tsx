...
function PaymentsPage() {
  const [search, setSearch] = useState(""); const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10); const [open, setOpen] = useState(false); const [invoiceId, setInvoiceId] = useState(""); const [amount, setAmount] = useState(""); const [method, setMethod] = useState("bank" as const);
  const { data } = usePayments({ search, page, pageSize }); const { data: invoices } = useInvoices({ pageSize: 50 }); const record = useRecordPayment();
  return <AppShell><PageHeader title="Payments" description="Record collections and keep customer balances up to date." crumbs={[{ label: "Home", to: "/" }, { label: "Payments" }]} actions={<Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button><CreditCard className="size-4" /> Record payment</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Record payment</DialogTitle></DialogHeader><div className="space-y-3"><div><TextField select label="Invoice" className="mt-1.5 h-9 w-full rounded-md border bg-background px-3 text-sm" value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)}><MenuItem value="">Select invoice</MenuItem>{invoices?.rows.filter((invoice) => invoice.balance > 0).map((invoice) => <MenuItem key={invoice.id} value={invoice.id}>{invoice.number} · {invoice.customerName} · {formatCurrency(invoice.balance)}</MenuItem>)}</TextField></div><div><TextField label="Amount" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} /></div><div><TextField select label="Method" className="mt-1.5 h-9 w-full rounded-md border bg-background px-3 text-sm" value={method} onChange={(e) => setMethod(e.target.value as typeof method)}><MenuItem value="bank">Bank transfer</MenuItem><MenuItem value="cash">Cash</MenuItem><MenuItem value="card">Card</MenuItem><MenuItem value="upi">UPI</MenuItem><MenuItem value="cheque">Cheque</MenuItem></TextField></div></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={!invoiceId || !amount || record.isPending} onClick={() => record.mutate({ invoiceId, amount: Number(amount), method }, { onSuccess: () => { toast.success("Payment recorded"); setOpen(false); setAmount(""); } })}>Save payment</Button></DialogFooter></DialogContent></Dialog>} /><Card className="p-0"><div className="border-b p-4"><SearchInput value={search} onChange={setSearch} placeholder="Search reference, customer or invoice" /></div><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Reference</TableHead><TableHead>Customer</TableHead><TableHead>Invoice</TableHead><TableHead>Date</TableHead><TableHead>Method</TableHead><TableHead className="text-right">Amount</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{data?.rows.map((payment) => <TableRow key={payment.id}><TableCell className="font-medium">{payment.reference}</TableCell><TableCell>{payment.partyName}</TableCell><TableCell>{payment.invoiceNumber}</TableCell><TableCell>{formatDate(payment.receivedAt)}</TableCell><TableCell className="capitalize">{payment.method}</TableCell><TableCell className="numeric text-right">{formatCurrency(payment.amount)}</TableCell><TableCell><StatusBadge status={payment.status} /></TableCell></TableRow>)}</TableBody></Table>  </div><Pagination page={page} pageSize={pageSize} total={data?.total ?? 0} onPageChange={setPage} onPageSizeChange={setPageSize} /></Card></AppShell>;
}
