...
function UsersPage() {
  ...
  return <AppShell>
    <PageHeader title="Users, roles & staff" description="Manage users, define access roles and assign roles to staff." crumbs={[{ label: "Home", to: "/" }, { label: "Users & roles" }]} actions={tab === "users" ? <Dialog open={userDialog} onOpenChange={setUserDialog}><DialogTrigger asChild><Button><Plus className="size-4" /> Invite user</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>{editingUser ? "Edit user" : "Invite team member"}</DialogTitle></DialogHeader><div className="space-y-4"><div><TextField label="Full name" className="mt-1.5" value={userForm.name} onChange={(event) => setUserForm({ ...userForm, name: event.target.value })} /></div><div><TextField label="Email address" className="mt-1.5" type="email" value={userForm.email} onChange={(event) => setUserForm({ ...userForm, email: event.target.value })} /></div><div><TextField select label="Role" className="mt-1.5 h-9 w-full rounded-md border bg-background px-3 text-sm" value={userForm.role} onChange={(event) => setUserForm({ ...userForm, role: event.target.value })}>{roleOptions.map((role) => <MenuItem key={role} value={role}>{role}</MenuItem>)}</TextField></div></div><DialogFooter><Button variant="outline" onClick={() => setUserDialog(false)}>Cancel</Button><Button disabled={!userForm.name || !userForm.email} onClick={saveUser}>{editingUser ? "Save changes" : "Send invitation"}</Button></DialogFooter></DialogContent></Dialog> : tab === "roles" ? <Dialog open={roleDialog} onOpenChange={setRoleDialog}><DialogTrigger asChild><Button><Plus className="size-4" /> New role</Button></DialogTrigger><DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{editingRole ? "Edit role" : "Create role"}</DialogTitle></DialogHeader><div className="space-y-4"><div><TextField label="Role name" className="mt-1.5" value={roleForm.name} onChange={(event) => setRoleForm({ ...roleForm, name: event.target.value })} disabled={editingRole?.system} /></div><div><TextField label="Description" className="mt-1.5" value={roleForm.description} onChange={(event) => setRoleForm({ ...roleForm, description: event.target.value })} /></div><div><p className="text-sm font-medium">Module permissions</p>    <div className="mt-2 max-h-[45vh] overflow-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>Scope</TableHead>{(["CREATE", "READ", "UPDATE", "DELETE"] as Operation[]).map((operation) => <TableHead key={operation} className="text-center">{operation}</TableHead>)}</TableRow></TableHeader><TableBody>{SCOPES.map((scope) => <TableRow key={scope}><TableCell className="font-medium">{scope}</TableCell>{(["CREATE", "READ", "UPDATE", "DELETE"] as Operation[]).map((operation) => <TableCell key={operation} className="text-center"><Checkbox size="small" aria-label={`${scope} ${operation}`} checked={roleForm.permissions[scope]?.includes(operation) ?? false} onChange={() => toggleOperation(scope, operation)} /></TableCell>)}</TableRow>)}</TableBody></Table></div></div></div><DialogFooter><Button variant="outline" onClick={() => setRoleDialog(false)}>Cancel</Button><Button disabled={!roleForm.name || Object.keys(roleForm.permissions).length === 0} onClick={saveRole}>Save role</Button></DialogFooter></DialogContent></Dialog> : <Dialog open={staffDialog} onOpenChange={setStaffDialog}><DialogTrigger asChild><Button><Plus className="size-4" /> Create staff</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>{editingStaff ? "Edit staff member" : "Create staff member"}</DialogTitle></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><div><TextField label="Full name" className="mt-1.5" value={staffForm.name} onChange={(event) => setStaffForm({ ...staffForm, name: event.target.value })} /></div><div><TextField label="Email" className="mt-1.5" type="email" value={staffForm.email} onChange={(event) => setStaffForm({ ...staffForm, email: event.target.value })} /></div><div><TextField label="Phone" className="mt-1.5" value={staffForm.phone} onChange={(event) => setStaffForm({ ...staffForm, phone: event.target.value })} /></div><div><TextField label="Department" className="mt-1.5" value={staffForm.department} onChange={(event) => setStaffForm({ ...staffForm, department: event.target.value })} /></div><div className="sm:col-span-2"><TextField select label="Assign role" className="mt-1.5 h-9 w-full rounded-md border bg-background px-3 text-sm" value={staffForm.role} onChange={(event) => setStaffForm({ ...staffForm, role: event.target.value })}>{roleOptions.map((role) => <MenuItem key={role} value={role}>{role}</MenuItem>)}</TextField></div></div><DialogFooter><Button variant="outline" onClick={() => setStaffDialog(false)}>Cancel</Button><Button disabled={!staffForm.name || !staffForm.email} onClick={saveStaff}>{editingStaff ? "Save changes" : "Create staff"}</Button></DialogFooter></DialogContent></Dialog>} />
