...
import { useCategories, useCreateProduct, useDeleteProduct, useProducts } from "@/hooks/queries/products";
import { formatCurrency, formatNumber } from "@/utils/format";

export const Route = createFileRoute("/products/")({ component: ProductsPage });

const EMPTY = { name: "", sku: "", barcode: "", categoryId: "cat-1", unit: "pcs", purchasePrice: "0", sellingPrice: "0", taxRate: "18", stock: "0", minStock: "0", warehouseId: "wh-1", status: "active" as const, description: "" };
  ...
  return <AppShell>
    <PageHeader title="Products" description="Manage your catalogue, pricing and product availability." crumbs={[{ label: "Home", to: "/" }, { label: "Products" }]} actions={
      <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button><Plus className="size-4" /> New product</Button></DialogTrigger>
        <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Add product</DialogTitle></DialogHeader>
          ...
          <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={!form.name || !form.sku || create.isPending} onClick={() => create.mutate({ ...form, purchasePrice: Number(form.purchasePrice), sellingPrice: Number(form.sellingPrice), taxRate: Number(form.taxRate), stock: Number(form.stock), minStock: Number(form.minStock) }, { onSuccess: () => { toast.success("Product added"); setOpen(false); setForm(EMPTY); } })}>Save product</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    } />
